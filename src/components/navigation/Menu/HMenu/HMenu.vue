...
import MenuNode from "./HMenuNode.vue";
import type { HMenuContext, HMenuInstance, HMenuItemModel, HMenuItemProps, HMenuProps } from "./types";
import { HMenuContextKey } from "./types";

defineOptions({
    name: "MHMenu"
});
const props = withDefaults(defineProps<HMenuProps>(), {
    router: false
});
...
const nodesMap = shallowReactive<Map<string | number, HMenuItemModel>>(new Map());
const updateBar = () => {
...
const currentPath = computed<Array<HMenuItemProps>>(() => {
    const path: Array<HMenuItemProps> = [];
...
provide<HMenuContext>(HMenuContextKey, {
