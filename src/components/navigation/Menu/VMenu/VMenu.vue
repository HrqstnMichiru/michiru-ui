...
import type { VMenuContext, VMenuInstance, VMenuItemModel, VMenuItemProps, VMenuProps } from "./types";
import { VMenuContextKey } from "./types";
...
const menuItems = shallowReactive<Map<string | number, VMenuItemModel>>(new Map()); // 存储所有菜单项数据
...
const register = (item: VMenuItemModel) => {
...
const currentPath = computed<Array<VMenuItemProps>>(() => {
    const path: Array<VMenuItemProps> = [];
...
provide<VMenuContext>(VMenuContextKey, {
