...
import type { TabContext, TabEmits, TabInstance, TabPanelProps, TabProps } from "./types";
...
const isReady = ref<boolean>(false); // 婵€娲绘潯鏄惁鍑嗗濂斤紝鐢ㄤ簬鎺у埗鍔ㄧ敾
const activeTab = ref<number | string>(props.defaultActive); // 褰撳墠婵€娲荤殑MTab鍚嶇О
const currentIndex = ref<number>(0); // 褰撳墠婵€娲荤殑MTab绱㈠紩
const switchDirection = ref<"left" | "right">("right"); // 鍒囨崲鏂瑰悜
const tabItemList = reactive<TabPanelProps[]>([]);
const itemRefs = new Map<string | number, HTMLElement>();
const isAnimating = ref(false); // 鏄惁姝ｅ湪鍔ㄧ敾涓?
...
const setActiveTab = (item: TabPanelProps, targetIndex: number) => {
...
const register = (item: TabPanelProps) => {
...
const addTab = (item: TabPanelProps) => {
...
provide<TabContext>(TabContextKey, {
    register,
    isActive,
    get switchDirection() {
