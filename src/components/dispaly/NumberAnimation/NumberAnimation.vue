...
import type { NumberAnimationEasing, NumberAnimationEmits, NumberAnimationInstance, NumberAnimationProps } from "./types";
...
const emits = defineEmits<NumberAnimationEmits>();

let rafId = 0;
const currentValue = ref<number>(props.from ?? 0);
const displayValue = computed(() => formatNumber(currentValue.value));
const easingMap: Record<NumberAnimationEasing, (t: number) => number> = {
    linear: t => t, // 匀速
...
defineExpose<NumberAnimationInstance>({
    start,
    stop
});
