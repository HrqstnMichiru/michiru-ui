...
export interface TreeNodeProps<T extends TreeData = any> {
    nodeKey: string | number; // 节点唯一标识
    label: string; // 节点显示文本
    children?: TreeNodeProps<T>[]; // 子节点
    parent?: TreeNodeProps<T>; // 父节点
...
export interface TreeEmits {
    <T extends TreeData = any>(e: "node-select", node: TreeNodeProps<T>): void;
    <T extends TreeData = any>(e: "node-click", node: TreeNodeProps<T>): void;
    <T extends TreeData = any>(e: "selection-change", selectedNodes: T[]): void;
}

export interface TreeProps<T extends TreeData = any> {
    data?: T[]; // 树节点数据
...
export interface TreeInstance<T extends TreeData = any> {
