...
import type { _MMessageInstance, _MMessageOptions, MMessageProviderContext, MMessageProviderProps, MMessageVariant } from "./types";
import { MMessageProviderContextKey } from "./types";

defineOptions({
    name: "MMessageProvider"
});
const props = withDefaults(defineProps<MMessageProviderProps>(), {
    duration: 3000,
    maxCount: 5,
    offset: 20,
    zIndex: 9999,
...
const instances = shallowReactive<_MMessageInstance[]>([]);
...
const createMessage = (options: _MMessageOptions): _MMessageInstance => {
    ...
    const instance: _MMessageInstance = {
        ...options,
        id,
        duration,
...
provide<MMessageProviderContext>(MMessageProviderContextKey, {
    create: createMessage,
    primary(message: string, props: Omit<_MMessageOptions, "variant" | "message"> = {}) {
        return createMessage({ ...props, message, variant: "primary" });
    },
    success(message: string, props: Omit<_MMessageOptions, "variant" | "message"> = {}) {
        return createMessage({ ...props, message, variant: "success" });
    },
    warning(message: string, props: Omit<_MMessageOptions, "variant" | "message"> = {}) {
        return createMessage({ ...props, message, variant: "warning" });
    },
    danger(message: string, props: Omit<_MMessageOptions, "variant" | "message"> = {}) {
        return createMessage({ ...props, message, variant: "danger" });
    },
    info(message: string, props: Omit<_MMessageOptions, "variant" | "message"> = {}) {
        return createMessage({ ...props, message, variant: "info" });
    },
    purple(message: string, props: Omit<_MMessageOptions, "variant" | "message"> = {}) {
        return createMessage({ ...props, message, variant: "purple" });
    },
    pink(message: string, props: Omit<_MMessageOptions, "variant" | "message"> = {}) {
