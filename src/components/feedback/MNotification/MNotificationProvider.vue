                ...
                <template v-for="instance in placementInstances" :key="instance.id">
                    <div
                        ...
                        <div class="notification__header">
                            ...
                            <span class="notification__close" v-if="instance.showClose" @click="destroyNotification(placement as MNotificationPlacement, instance.id)">
...
import type { _MNotificationInstance, _MNotificationOptions, MNotificationPlacement, MNotificationProviderContext, MNotificationProviderProps, MNotificationVariant } from "./types";
import { MNotificationProviderContextKey } from "./types";

defineOptions({
    name: "MNotificationProvider"
});
const props = withDefaults(defineProps<MNotificationProviderProps>(), {
    zIndex: 2000,
    offset: 20,
    duration: 4500,
    gap: 20,
...
const instances: Record<MNotificationPlacement, ShallowReactive<_MNotificationInstance[]>> = {
...
const containerStyle = (placement: MNotificationPlacement) => {
...
const createNotification = (options: _MNotificationOptions): _MNotificationInstance => {
    ...
    const instance: _MNotificationInstance = {
        showIcon: true,
        showClose: true,
        ...options,
        id,
...
const destroyNotification = (placement: MNotificationPlacement, id: string) => {
...
const startTimer = (placement: MNotificationPlacement, id: string, duration: number) => {
...
provide<MNotificationProviderContext>(MNotificationProviderContextKey, {
    create: createNotification,
    primary: (message: string, options?: Omit<_MNotificationOptions, "message" | "variant">) => createNotification({ ...options, message, variant: "primary" }),
    success: (message: string, options?: Omit<_MNotificationOptions, "message" | "variant">) => createNotification({ ...options, message, variant: "success" }),
    warning: (message: string, options?: Omit<_MNotificationOptions, "message" | "variant">) => createNotification({ ...options, message, variant: "warning" }),
    info: (message: string, options?: Omit<_MNotificationOptions, "message" | "variant">) => createNotification({ ...options, message, variant: "info" }),
    danger: (message: string, options?: Omit<_MNotificationOptions, "message" | "variant">) => createNotification({ ...options, message, variant: "danger" }),
    destroyAll: () => {
