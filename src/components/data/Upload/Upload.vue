...
import type { FormContext, FormItemContext, MImagePreviewInstance } from "@/components";
import { FormContextKey, FormItemContextKey, MIcon, MImagePreview } from "@/components";
import { nanoid } from "nanoid";
import { computed, inject, onBeforeMount, reactive, ref, useTemplateRef } from "vue";
import type { UploadEmits, UploadFile, UploadInstance, UploadProps } from "./types";
...
const uploadFileList = reactive<UploadFile[]>([]);
...
const handleDrop = async (e: DragEvent) => {
    ...
    if (inputRef.value) {
        inputRef.value.value = ""; // 娓呯┖閫夋嫨鐨勬枃浠剁殑铏氭嫙璺緞
    }
};
...
const reUploadFile = (file: UploadFile) => {
...
onBeforeMount(() => {
    if (props.listStyle === "gallery" && modelValue.value) {
        if (Array.isArray(modelValue.value)) {
            const initialFiles: UploadFile[] = modelValue.value.map(url => {
                return {
                    fid: url,
                    type: "image/",
                    status: "done",
