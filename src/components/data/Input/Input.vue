...
let formItemContext: FormItemContext | null = null;
let formContext: FormContext | null = null;
if (props.injected) {
    formItemContext = inject<FormItemContext | null>(FormItemContextKey, null);
    formContext = inject<FormContext | null>(FormContextKey, null);
}
const customSize = computed(() => {
    const result = formItemContext?.size || props.size || "medium";
    return result;
});
...
const handleBlur = () => {
    if (props.disabled) return;
    emits("blur");
    if (formItemContext && formItemContext.trigger === "blur" && props.injected) {
